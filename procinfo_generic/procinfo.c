...
static struct kprobe jprobes[JPROBE_TOTAL];
...
static int linuxdrv_init(void)
{
    ...
    /*
    jprobes[0].kp.symbol_name = "free_pipe_info";
    jprobes[0].entry = (void*)linuxdrv_finish_fork;
    jprobes[1].kp.symbol_name = "exit_io_context";
    jprobes[1].entry = (void*)linuxdrv_finish_exit;
    jprobes[2].kp.symbol_name = "free_bprm";
    ...
    */
    ...
    jprobes[0].symbol_name = "do_exit";
    jprobes[1].symbol_name = "exit_io_context";
    jprobes[2].symbol_name = "free_bprm";
    jprobes[3].symbol_name = "vma_link";
    jprobes[4].symbol_name = "vma_adjust";
    jprobes[5].symbol_name = "remove_vma";
    jprobes[6].symbol_name = "modules";
    jprobes[7].symbol_name = "trim_init_extable";
...
*/
    ...
    for(i = 0; i < JPROBE_TOTAL; i++) {
        ...
        jprobes[i].pre_handler = (void*)handler_pre;
        //register_kjprobe_returnprobe(&jprobes[i]);
        register_kprobe(&jprobes[i]);
    }

    for(i = 0; i < JPROBE_TOTAL; i++)  {
        if(jprobes[i].addr != NULL)  {
            printk(KERN_INFO
            "%s  = %lu\n",
            jprobes[i].symbol_name,
            jprobes[i].addr);
        }
    }
